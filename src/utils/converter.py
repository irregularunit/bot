...
custom_emoji: re.Pattern[str] = re.compile(r"<(?P<a>a)?:(?P<name>[a-zA-Z0-9_~]{1,}):(?P<id>[0-9]{15,19})>")
...
class EmojiConverter(commands.Converter[discord.Emoji]):
    ...
    async def from_message(self, ctx: Context, message: discord.Message) -> Optional[list[discord.PartialEmoji]]:
        ...
        message_emojis: Optional[list[tuple[str, str, str]]] = custom_emoji.findall(message_content)
    ...
    async def convert(self, ctx: Context, argument: Optional[str]) -> Optional[list[discord.PartialEmoji]]:
        ...
        if argument is not None:
            for emoji in argument.split():
                try:
                    partial_emoji: discord.PartialEmoji = await commands.PartialEmojiConverter().convert(ctx, emoji)
                    emojis.append(partial_emoji)
                except commands.PartialEmojiConversionFailure:
                    continue
        ...
        if not emojis:
            async for message in ctx.channel.history(limit=10):
                maybe_found: list[discord.PartialEmoji] | None = await self.from_message(ctx, message)
                if maybe_found:
