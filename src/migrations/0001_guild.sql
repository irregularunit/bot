CREATE TABLE IF NOT EXISTS serenity_guilds (
    ...
    CONSTRAINT serenity_guilds_pkey UNIQUE (snowflake)
);

CREATE TABLE IF NOT EXISTS serenity_guild_prefixes (
    snowflake BIGINT NOT NULL,
    prefix VARCHAR(2000) NOT NULL,
    CONSTRAINT serenity_guild_prefixes_fkey FOREIGN KEY (snowflake) REFERENCES serenity_guilds(snowflake) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS serenity_guild_emotes (
    ...
    CONSTRAINT serenity_guild_emotes_fkey FOREIGN KEY (snowflake) REFERENCES serenity_guilds(snowflake) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS serenity_guild_snipes (
    ...
    CONSTRAINT serenity_guild_snipes_fkey FOREIGN KEY (snowflake) REFERENCES serenity_guilds(snowflake) ON DELETE CASCADE,
    CONSTRAINT serenity_snipes_message_pkey UNIQUE (message_snowflake)
);

CREATE INDEX IF NOT EXISTS serenity_guild_snipes_channel_snowflake_index ON serenity_guild_snipes (channel_snowflake);
CREATE INDEX IF NOT EXISTS serenity_guild_emotes_emote_usage_index ON serenity_guild_emotes (emote_usage);
